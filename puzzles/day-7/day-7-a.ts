...
function typeOfHand(dist: CardDistribution): HandType {
  ...
  const hasCount = (count: number): boolean => {
    return cards.some(c => dist[c] === count);
  }
  if (hasCount(5)) { return 'five-kind'; }
  if (hasCount(4)) { return 'four-kind'; }
  if (hasCount(3) && hasCount(2)) { return 'full-house'; }
  if (hasCount(3)) { return 'three-kind'; }
  if (hasCount(2)) {
    //check for two-pair
...
export async function day7a(dataPath?: string) {
  ...
  if (Verbose.isActive()) {
    matrices.forEach((m, index) => {
      verbose.add(`hand: ${m.hand.join('')}, bid: ${m.bid}, type: ${m.type}, winnings: ${(index+1)*m.bid}`).display();
    });
  }
