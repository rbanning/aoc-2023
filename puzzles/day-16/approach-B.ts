...
import { Coordinate, CoordinateDelta, Direction, deltaDirection } from "./direction.ts";
...
export function generateStartingPoints(grid: Grid): StartingPoint[] {
  const points: StartingPoint[] = [];

  for (let row = 0; row < grid.length; row++) {
    points.push({ coord: [row, 0], delta: [0, 1] });                      //from the left
    points.push({ coord: [row, grid[0].length-1], delta: [0, -1] });  //from the right
  }
  for (let col = 0; col < grid[0].length; col++) {
    points.push({ coord: [0, col], delta: [1, 0] });                  //from the top
    points.push({ coord: [grid[0].length-1, col], delta: [-1, 0] });  //from the bottom
  }

  //return [{ coord: [0, 3], delta: [1, 0] }];
  return points;
}
...
function cacheKey(coord: Coordinate, direction: Direction) {
